...
const WorkspaceDropdown: React.FC<Props> = ({ className, current }) => {
	...
	const [workspaceSelected, setWorkspaceSelected] = useState<Workspace>(current)
	const workspaces = useWorkspaces()

	return (
		<div className={clsx(className)}>
			{workspaces.isSuccess && (
				<Listbox className='relative' as='div' value={workspaceSelected} onChange={setWorkspaceSelected}>
					<Listbox.Button className='flex items-center'>
						<div className='mr-1'>{current.name ?? <Skeleton width='10rem' />}</div>
